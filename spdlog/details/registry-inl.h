...
inline void registry::flush_every(std::chrono::seconds interval)
{
...
inline void registry::apply_all(const std::function<void(const std::shared_ptr<logger>)> &fun)
{
    std::lock_guard<std::mutex> lock(logger_map_mutex_);
    for (auto &l : loggers_)
    {
...
inline std::recursive_mutex &registry::tp_mutex()
{
    return tp_mutex_;
}
